...
function Audit-ATPPolicies {
    ...
    catch { Write-Status "Safe Links não disponível (requer Defender for Office 365)" "Detail" }
    ...
    catch { Write-Status "Safe Attachments não disponível" "Detail" }
    ...
    catch { Write-Status "Anti-Phishing check falhou" "Detail" }
}
...
function Audit-AuditLog {
    ...
    try {
        ...
        if ($mailboxAudit) { Write-Status "✅ Mailbox Audit by Default: HABILITADO" "Success" }
        else { Write-Status "❌ Mailbox Audit by Default: DESABILITADO" "Error" }
    }
    catch { Write-Status "Erro: $($_.Exception.Message)" "Warning" }
    ...
    catch { Write-Status "⚠️ Unified Audit Log: não verificável ou desabilitado" "Warning" }
}
...
function Audit-TransportRules {
    ...
    catch { Write-Status "Erro Transport Rules: $($_.Exception.Message)" "Warning" }
}
...
function Audit-DKIM {
    ...
    catch { Write-Status "Erro DKIM: $($_.Exception.Message)" "Warning" }
}
...
function Export-Evidence {
    ...
    $md = "# Evidências de Políticas Implementadas - Purview Compliance Manager`n`n"
    $md += "**Tenant:** $tenantLabel`n**Data:** $(Get-Date -Format 'dd/MM/yyyy HH:mm')`n"
    $md += "**Total de evidências:** $($Script:Evidence.Count)`n`n"
    $md += "## Resumo por Categoria`n`n| Categoria | Implementadas |`n|-----------|:------------:|`n"
