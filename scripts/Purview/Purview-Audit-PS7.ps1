...
function Get-DLPAudit {
    ...
    try {
        ...
        if ($null -eq $Policies -or @($Policies).Count -eq 0) {
            ...
            $Result.Recommendations += @{ Priority = "Critical"; Category = "DLP"; Message = "Nenhuma política DLP encontrada."; Remediation = "Acesse Purview > Data Loss Prevention > Policies > Create Policy" }
            return $Result
        }
    ...
    try {
        ...
        try {
            ...
            if ($ErrorMsg -match "UnifiedAuditLogIngestionEnabled.*False|not enabled|audit logging is not enabled") {
                ...
                $Result.Recommendations += @{ Priority = "Critical"; Category = "AuditLog"; Message = "🚨 CRÍTICO: Unified Audit Log está DESABILITADO!"; Remediation = "Execute o script M365-Remediation.ps1 ou ative manualmente no Purview" }
            }
            else {
        ...
        try {
            ...
            if ($Result.MailboxAuditEnabled) {
                ...
                $Result.Recommendations += @{ Priority = "High"; Category = "AuditLog"; Message = "Mailbox Audit por padrão está desabilitado"; Remediation = "Execute: Set-OrganizationConfig -AuditDisabled `$false" }
            }
        }
        catch {
...
function Get-RetentionAudit {
    ...
    try {
        ...
        if ($null -eq $Policies -or @($Policies).Count -eq 0) {
            ...
            $Result.Recommendations += @{ Priority = "High"; Category = "Retention"; Message = "Nenhuma política de retenção configurada"; Remediation = "Configure políticas de retenção para compliance" }
        }
        else {
            $Result.Details.TotalPolicies = @($Policies).Count
...
function Get-SensitivityLabelsAudit {
    ...
    try {
        ...
        if ($null -eq $Labels -or @($Labels).Count -eq 0) {
            ...
            $Result.Recommendations += @{ Priority = "High"; Category = "SensitivityLabels"; Message = "Nenhum label de sensibilidade configurado"; Remediation = "Configure labels para classificação e proteção de dados" }
            return $Result
        }
        $Result.Details.TotalLabels = @($Labels).Count
...
function Get-ExternalSharingAudit {
    ...
    try {
        ...
        if ($Result.Score -lt 100) {
            $Result.Recommendations += @{ Priority = "Low"; Category = "ExternalSharing"; Message = "Algumas políticas permitem serviços externos no OWA"; Remediation = "Avalie se é necessário permitir provedores externos" }
        }
    }
    catch {
        Write-Status "Erro: $($_.Exception.Message)" "Warning"
...
"@
    ...
    if ($GenerateHTML) {
        ...
        foreach ($Cat in $Categories) {
            if ($Results[$Cat]) {
                if ($Results[$Cat].Skipped) {
                    $CategoryRows += "<tr class='na'><td>$Cat</td><td>N/A</td><td>Não licenciado</td></tr>"
                }
                else {
                    $Score = $Results[$Cat].Score
        ...
        $SkippedHtml = ""
        if ($Script:SkippedCategories.Count -gt 0) {
            $SkippedHtml = "<h2>Categorias puladas (não licenciadas)</h2><ul>" + ($Script:SkippedCategories | ForEach-Object { "<li>$_</li>" }) -join "" + "</ul>"
        }
...
<body>
    ...
    <div class="meta">Data: $(Get-Date -Format "dd/MM/yyyy HH:mm")<br/>Tenant: $TenantName<br/>Licença: $LicenseInfo</div>
...
function Show-Summary {
    ...
    Write-Host "  ─────────────────────────────────────────────" -ForegroundColor Gray
    if ($CriticalCount -gt 0) { Write-Host "  🚨 Críticas: $CriticalCount" -ForegroundColor Red }
    if ($HighCount -gt 0) { Write-Host "  ⚠️  Alta: $HighCount" -ForegroundColor Yellow }
    if ($MediumCount -gt 0) { Write-Host "  ℹ️  Média/Baixa: $MediumCount" -ForegroundColor White }
    if (($CriticalCount + $HighCount + $MediumCount) -eq 0) { Write-Host "  ✅ Nenhuma recomendação crítica!" -ForegroundColor Green }
    Write-Host ""
}
